...
public class KubernetesManager : IContainerManager
{
    ...
    public async Task<Models.Data.Container?> CreateVpnContainerAsync(ContainerConfig config,
        CancellationToken token = default)
    {
        var imageName = "OpenVPN";
        var authSecretName = _meta.AuthSecretName;
        KubernetesConfig options = _meta.Config;

        var chalImage = imageName.ToValidRFC1123String("chal");

        var name = $"vpn-{config.TeamId}-{config.UserId.ToString()}-{Ulid.NewUlid().ToString().ToLowerInvariant()}";
