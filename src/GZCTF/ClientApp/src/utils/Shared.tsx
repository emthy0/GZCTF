...
export const getProxyUrl = (guid: string, isPreview: boolean = false, publicEntry?: string) => {
  const protocol = window.location.protocol.replace('http', 'ws')
  const api = isPreview ? 'api/proxy/noinst' : 'api/proxy'
  const host = publicEntry ?? window.location.host
  return `${protocol}//${host}/${api}/${guid}`
}
